...
function getMockData(location?: string): any[] {
  ...
  return [
    {
      id: 'twitter_mock_001', // Consistent ID
    ...
    {
      id: 'twitter_mock_002', // Consistent ID
...
async function savePostsToDatabase(posts: any[]) {
  ...
  for (const post of posts) {
    ...
    const exists = await checkExistingPost('twitter', externalId)
    if (exists) {
...
Deno.serve(async (req) => {
  try {
    ...
    return new Response(
      JSON.stringify({
        success: true,
        message: `Successfully scraped and saved ${savedPosts?.length || 0} new posts`,
        location: location,
